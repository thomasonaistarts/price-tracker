...
if ($WolvoxHost -notin @('127.0.0.1', 'localhost', '::1')) {
  throw 'This first depot read only permits the local WOLVOX service.'
}
...
try {
  Write-Host 'Fiyatlaa WOLVOX 26 read-only depot export' -ForegroundColor Cyan
  ...
  $depotXml = Invoke-WolvoxSdk ([ordered]@{
    command     = 'get_depolist'
    tPwd        = $temporaryPassword
    sirketKodu  = $CompanyCode
    calismaYili = $WorkingYear
  })

  if (-not $depotXml.TrimStart().StartsWith('<')) {
    throw 'WOLVOX did not return depot data in XML format.'
  }

  try {
    [xml]$parsedDepotXml = $depotXml
  }
  catch {
    throw "WOLVOX returned invalid depot XML: $($_.Exception.Message)"
  }

  $outputDirectory = Join-Path $env:TEMP 'fiyatlaa-wolvox'
  New-Item -ItemType Directory -Path $outputDirectory -Force | Out-Null
  $outputPath = Join-Path $outputDirectory ('depot-list-{0}-{1}-{2}.xml' -f $CompanyCode, $WorkingYear, (Get-Date -Format 'yyyyMMdd-HHmmss'))
  [IO.File]::WriteAllText($outputPath, $depotXml, [Text.UTF8Encoding]::new($false))

  $rowCount = @($parsedDepotXml.report.table.row).Count
  Write-Host 'OK: Read-only WOLVOX 26 session opened.' -ForegroundColor Green
  Write-Host "OK: Depot list XML received ($rowCount records)." -ForegroundColor Green
